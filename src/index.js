...
import { grantDomainRewards, levelCharacter, upgradeTalent, setShowcase, upgradePassive, equipWeapon, equipArtifact } from './game/progression.js';
...
function getSession(userId) {
  if (!sessions.has(userId)) sessions.set(userId,{ view:'home', banner:'character', team:[], domainId:null, battle:null, pendingAction:null, lastWish:[], selectedChar:null, avatarUrl:null, lastDomainReward:null });
...
function componentsFor(player,session,userId) {
  ...
  switch(session.view){
    ...
    case 'profile': return homeComponents(userId);
...
async function payloadFor(player,session,userId,user,editing=false) {
  ...
  try{
    ...
    if(elapsed>500) console.log(`[render] ${session.view}: ${elapsed}ms`);
...
async function refreshPersistentProfile(channel,player,user,skipMessageId=null) {
  const messageId=player.ui?.profileMessageId;
  if(!messageId || messageId===skipMessageId) return;
  try{
    const msg=await channel.messages.fetch(messageId);
    const live=getSession(user.id);
    const profileSession={...live,view:'home',battle:null,pendingAction:null,selectedChar:null};
    await msg.edit(await payloadFor(player,profileSession,user.id,user,true));
  }catch(error){
    if(error?.code===10008 || error?.status===404) {
      store.mutatePlayer(user.id,(p)=>{p.ui.profileMessageId=null;},user.username);
    } else console.error('Profile refresh failed:',error.message);
  }
}

async function openOrRefreshProfile(message,player,session) {
...
function actionAnnouncement(userId,event,isCpu=false){
  ...
  line+=` • Talent Lv.${event.talentLevel||1}`;
...
client.once(Events.ClientReady,(ready)=>console.log(`Neverless RPG V2 online as ${ready.user.tag}. Game channel: ${GAME_CHANNEL_ID}`));
...
client.on(Events.MessageCreate,async(message)=>{
  ...
  Object.assign(session,{view:'home',battle:null,pendingAction:null,selectedChar:null,lastWish:[],lastDomainReward:null,avatarUrl:message.author.displayAvatarURL({extension:'png',size:256})});
  await openOrRefreshProfile(message,player,session);
});
...
client.on(Events.InteractionCreate,async(interaction)=>{
  ...
  if(interaction.user.id!==parsed.ownerId){await interaction.reply({content:'This RPG panel belongs to another player. Use `-neverless` to open your own profile.',ephemeral:true});return;}

  const userId=interaction.user.id,player=store.getPlayer(userId,interaction.user.username),session=getSession(userId);
  session.avatarUrl=interaction.user.displayAvatarURL({extension:'png',size:256});
  const persistentProfile=player.ui?.profileMessageId===interaction.message.id;
  const openingSubpanel=parsed.action==='mainmenu'&&persistentProfile&&(interaction.values?.[0]||'profile')!=='profile';
  if(openingSubpanel)await interaction.deferReply();else await interaction.deferUpdate();
  let announcements=[];

  try{
    if(parsed.action==='mainmenu'){
      const next=interaction.values[0]||'home';
      session.view=next==='profile'?'home':next;session.selectedChar=null;session.pendingAction=null;session.battle=null;
      if(session.view!=='domains'&&session.view!=='cpu')session.team=[];
      if(openingSubpanel){await interaction.editReply(await payloadFor(player,session,userId,interaction.user,false));return;}
    }else if(parsed.action==='view'){
      session.view=parsed.arg||'home';session.selectedChar=null;session.pendingAction=null;
      if(session.view==='home')session.battle=null;
      if(session.view!=='domains'&&session.view!=='cpu')session.team=[];
      ...
      session.domainId=interaction.values[0];session.team=[];session.lastDomainReward=null;
      ...
      if(calculatePlayerPower(player)<Math.ceil(domain.recommendedPower*1.12))throw new Error('POWER_TOO_LOW');
      if(player.resin<domain.resin)throw new Error('NOT_ENOUGH_RESIN');
      session.lastDomainReward=store.mutatePlayer(userId,(p)=>{p.resin-=domain.resin;return grantDomainRewards(p,domain.id,Math.random,{quick:true});},interaction.user.username);
      announcements.push(`<@${userId}> نفذ **Quick Clear** في **${domain.name}** • +${session.lastDomainReward.mora.toLocaleString()} Mora • +${session.lastDomainReward.primogems} Primogems • ${session.lastDomainReward.set.name}.`);
      ...
      store.mutatePlayer(userId,(p)=>setShowcase(p,interaction.values),interaction.user.username);
      announcements.push('تم تحديث الشخصيات الظاهرة في البروفايل.');
      ...
      const result=store.mutatePlayer(userId,(p)=>upgradePassive(p,parsed.arg),interaction.user.username);if(!result.ok)throw new Error(result.reason);
      ...
      const actor=currentActor(session.battle),result=act(session.battle,actor.uid,parsed.arg,interaction.values[0]);if(!result.ok)throw new Error(result.reason);
      ...
      if(session.battle.winner){
        ...
        const rewardText=store.mutatePlayer(userId,(p)=>{
          ...
          if(battle.winner==='player'&&battle.mode==='domain'){
            ...
            session.lastDomainReward=reward;
            return `Rewards: **${reward.mora.toLocaleString()} Mora**, **${reward.primogems} Primogems**, **${reward.adventureXp} XP**, ${reward.artifact.rarity}★ **${reward.set.name} ${reward.artifact.slot}**.`;
          }
          if(battle.winner==='player'&&battle.mode==='cpu')return 'Rewards: **8,000 Mora**, **80 XP** and **+18 MMR**.';
          return 'No victory rewards.';
        },interaction.user.username);
        battle.log.unshift(rewardText);announcements.push(`**${battle.winner==='player'?'Victory':'Defeat'}** • ${rewardText}`);
      }
    ...
    await refreshPersistentProfile(interaction.channel,player,interaction.user,interaction.message.id);
    if(announcements.length)await interaction.followUp({content:announcements.filter(Boolean).join('\n').slice(0,1900)});
  }catch(error){
    console.error(error);
    const friendly={
      NOT_ENOUGH_PRIMOGEMS:'Not enough Primogems.',NOT_ENOUGH_RESIN:'Not enough Resin.',NO_MORA:'Not enough Mora.',
      COOLDOWN:'That skill is still on cooldown.',NO_ENERGY:'Not enough Energy for Burst.',WRONG_WEAPON_TYPE:'That weapon type does not match the character.',
      MAX_LEVEL:'Character is already Lv.90.',TALENT_MAX:'That talent is already Lv.10.',TEAM_REQUIRED:'Choose exactly four characters first.',
      NO_DOMAIN:'Choose a Domain first.',QUICK_LOCKED:'Quick Clear unlocks after your first normal clear.',POWER_TOO_LOW:'Your account PWR is too low for Quick Clear.',CAP:'That passive upgrade reached its cap.',
