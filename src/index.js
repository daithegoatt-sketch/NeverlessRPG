...
const ERR={NOT_ENOUGH_PRIMOGEMS:'Not enough Primogems.',NOT_ENOUGH_RESIN:'Not enough Resin.',NO_MORA:'Not enough Mora.',MAX_LEVEL:'Character is already Lv.90.',TALENT_MAX:'Talent is already Lv.10.',WRONG_WEAPON_TYPE:'Wrong weapon type.',NO_DOMAIN:'Choose a Domain first.',TEAM_REQUIRED:'Choose exactly four characters.',QUICK_LOCKED:'Clear this Domain once before Quick Clear.',POWER_TOO_LOW:'Your PWR is too low for Quick Clear.',DAILY_ALREADY_CLAIMED:'Daily reward already claimed.',COOLDOWN:'Skill is on cooldown.',NO_ENERGY:'Not enough Energy for Burst.'};

client.once(Events.ClientReady,r=>console.log(`Neverless RPG V4 online as ${r.user.tag} • channel ${GAME_CHANNEL_ID}`));
...
client.on(Events.InteractionCreate,async interaction=>{
 if(!interaction.isButton()&&!interaction.isStringSelectMenu())return;if(interaction.channelId!==GAME_CHANNEL_ID){await interaction.reply({content:`Use <#${GAME_CHANNEL_ID}> for Neverless RPG.`,ephemeral:true});return;}
 ...
 try{
  ...
  else if(q.action==='daily'){const r=store.mutatePlayer(userId,x=>claimDaily(x),interaction.user.username);if(!r.ok)throw new Error(r.reason);notice=`Daily claimed: +${r.primogems} Primogems, +${r.mora.toLocaleString()} Mora, +${r.resin} Resin.`;resetHome(s);}
  ...
  else if(q.action==='levelchar'){const [id,n]=q.arg.split(':');const r=store.mutatePlayer(userId,x=>levelCharacter(x,id,Number(n)||1),interaction.user.username);if(!r.ok)throw new Error(r.reason);s.selectedChar=id;notice=`${characters[id].name} reached Lv.${r.level}. PWR increased.`;}
  else if(q.action==='talent'){const [id,t]=q.arg.split(':');const r=store.mutatePlayer(userId,x=>upgradeTalent(x,id,t),interaction.user.username);if(!r.ok)throw new Error(r.reason);s.selectedChar=id;notice=`${characters[id].name} ${t} talent reached Lv.${r.level}.`;}
