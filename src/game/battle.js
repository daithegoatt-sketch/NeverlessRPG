...
function playerActor(player, charId, side = 'player') {
  ...
  return {
    ...
    image: built.definition.image,
    element: built.definition.element,
    role: built.definition.role,
    skills: structuredClone(built.definition.skills),
    maxHp: built.stats.hp,
    hp: built.stats.hp,
    atk: built.stats.atk,
    def: built.stats.def,
    spd: built.stats.spd,
    critRate: built.stats.critRate,
    critDmg: built.stats.critDmg,
    elementDmg: built.stats.elementDmg || 0,
    energy: 0,
    shield: 0,
    atkBuff: 0,
    dmgTakenAmp: 0,
...
function enemyActor(enemy, idx) {
  return {
    uid: `enemy:${idx}:${enemy.id}`,
    side: 'enemy', id: enemy.id, name: enemy.name, image: enemy.image,
    element: enemy.element, role: 'Enemy',
    maxHp: enemy.hp, hp: enemy.hp, atk: enemy.atk, def: enemy.def, spd: enemy.spd,
    critRate: 0.08, critDmg: 0.50, elementDmg: 0, energy: 0, shield: 0, atkBuff: 0, dmgTakenAmp: 0,
    cooldowns: { skill: 0 },
    skills: {
      normal: { name: 'Attack', type: 'damage', multiplier: 1.00, energy: 25 },
      skill: { name: 'Heavy Attack', type: 'damage', multiplier: 1.45, energy: 35, cooldown: 1 },
      burst: { name: 'Rage', type: 'damage', multiplier: 2.10, cost: 80 },
    },
  };
}
...
export function createDomainBattle(player, teamIds, domain) {
  return createBattle({
    mode: 'domain',
    playerTeam: teamIds.map((id) => playerActor(player, id)),
    enemyTeam: domain.enemies.map(enemyActor),
...
export function createCpuBattle(player, teamIds, cpuPlayer) {
  ...
  return createBattle({
    mode: 'cpu',
    playerTeam: teamIds.map((id) => playerActor(player, id)),
    enemyTeam: cpuIds.map((id) => playerActor(cpuPlayer, id, 'enemy')),
    meta: { opponentName: 'Neverless Automaton' },
  });
}

export function createBattle({ mode, playerTeam, enemyTeam, meta = {} }) {
  const battle = {
    id: `b_${Date.now()}_${Math.floor(Math.random() * 1e7)}`,
    mode,
    turn: 0,
    tick: 0,
    actors: [...playerTeam, ...enemyTeam],
...
export function act(battle, actorUid, action, targetUid, rng = Math.random) {
  ...
  if (['damage', 'damageShield', 'damageDebuff', 'damageTeamBuff', 'selfBuffDamage', 'damageHeal'].includes(skill.type)) {
    ...
    const { damage, crit } = damageRoll(actor, target, skill.multiplier, rng);
    const dealt = applyDamage(target, damage);
    line = `${actor.name} used **${skill.name}** on ${target.name}: **${dealt} DMG**${crit ? ' ✨ CRIT' : ''}.`;

    if (skill.type === 'damageShield') actor.shield += Math.round(actor.maxHp * 0.15);
    if (skill.type === 'damageDebuff') target.dmgTakenAmp = clamp(target.dmgTakenAmp + 0.10, 0, 0.35);
    if (skill.type === 'damageTeamBuff') for (const a of friends) a.atkBuff = clamp(a.atkBuff + 0.10, 0, 0.40);
    if (skill.type === 'selfBuffDamage') actor.atkBuff = clamp(actor.atkBuff + 0.20, 0, 0.50);
    if (skill.type === 'damageHeal') {
      const h = heal(actor, actor.maxHp * 0.18);
      line += ` Healed ${h}.`;
    }
  } else if (skill.type === 'heal') {
    if (!target || target.side !== actor.side) target = friends.sort((a, b) => a.hp / a.maxHp - b.hp / b.maxHp)[0];
    const h = heal(target, target.maxHp * skill.multiplier);
    line = `${actor.name} used **${skill.name}** and healed ${target.name} for **${h} HP**.`;
  } else if (skill.type === 'teamHeal') {
    let total = 0;
    for (const a of friends) total += heal(a, a.maxHp * skill.multiplier);
    line = `${actor.name} used **${skill.name}** and restored **${total} team HP**.`;
  } else if (skill.type === 'teamBuffHeal') {
    let total = 0;
    for (const a of friends) {
      total += heal(a, a.maxHp * skill.multiplier);
      a.atkBuff = clamp(a.atkBuff + 0.18, 0, 0.50);
    }
    line = `${actor.name} used **${skill.name}**: team healed **${total} HP** and gained ATK.`;
  } else if (skill.type === 'shield') {
    actor.shield += Math.round(actor.maxHp * skill.multiplier);
    line = `${actor.name} used **${skill.name}** and gained **${Math.round(actor.maxHp * skill.multiplier)} shield**.`;
...
export function currentActor(battle) {
  return battle.actors.find((a) => a.uid === battle.pendingActorUid) || null;
}
