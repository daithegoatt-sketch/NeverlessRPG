...
export function homeComponents(userId) {
  const menu = new StringSelectMenuBuilder().setCustomId(cid(userId,'mainmenu')).setPlaceholder('Open Neverless RPG menu').addOptions([
    {label:'Wish',description:'Characters and weapons',value:'wish',emoji:'🌠'},
    {label:'Domains',description:'Farm Artifacts and account XP',value:'domains',emoji:'🏛️'},
    {label:'CPU Arena',description:'Fight Neverless Automaton',value:'cpu',emoji:'⚔️'},
    {label:'Characters',description:'Roster, skills, power and equipment',value:'characters',emoji:'🧙'},
    ...
    {label:'Profile',description:'Return to the profile dashboard',value:'profile',emoji:'👤'},
  ]);
  return [new ActionRowBuilder().addComponents(menu)];
}

export function backRow(userId,to='home') { return [new ActionRowBuilder().addComponents(btn(userId,'view','Back',ButtonStyle.Secondary,'⬅️',to))]; }
...
export function domainComponents(userId,player) {
  const menu = new StringSelectMenuBuilder().setCustomId(cid(userId,'domainselect')).setPlaceholder('Choose a domain').addOptions(
    Object.values(domains).map((d)=>({label:d.name,description:`PWR ${d.recommendedPower.toLocaleString()} • ${d.resin} Resin`,value:d.id}))
  );
...
export function weaponEquipComponents(userId,player,charId) {
  ...
  const options=Object.keys(player.weapons).filter((id)=>weapons[id]?.type===type).slice(0,25)
    .map((id)=>({label:weapons[id].name,description:`${'⭐'.repeat(weapons[id].rarity)} • ${type} • Lv.${player.weapons[id].level}`,value:id}));
  const rows=[];
...
export function artifactEquipComponents(userId,player,charId) {
  const options=Object.values(player.artifacts).slice(0,25).map((a)=>({label:`${a.slot} • ${a.setId}`,description:`${'⭐'.repeat(a.rarity)} +${a.level} • ${a.main.key}`,value:a.id}));
  const rows=[];
